...
from app.contracts.models import AgentRegistration,Digest,content_hash,utc_now
...
RESERVATIONS={'final_audit_model_calls':384,'external_model_calls':288,'comparison_actor_calls':384,'comparison_selector_calls':8,'model_calls':1064,'tokens':10640000}
...
def freeze_core(directory,identity,*,accepted_policy_hash=None,accepted_policy_version=None):
    ...
    if path.exists():
        ...
        if previous.get('status')=='FROZEN':
            if previous['identity']==identity and previous['learned_arm']==value['learned_arm'] and previous['final_manifest_hash']==value['final_manifest_hash']:return verify_freeze(directory)
            raise ValueError('immutable freeze already exists')
