...
Inference and a completed Weave connection trace were verified on 2026-09-12 Pacific. The selected model is `openai/gpt-oss-120b`; W&B's canonical project is `shreetbohara-quinstreet/Faultlab`. The user authorized a full learning cycle up to $100 and explicitly deferred Aria. Actual evidence and retained failures are in `docs/sponsor-results.md` and `docs/learning-results.md`.
...
| FAULTLAB_TOKEN_CAP | 60000000 |
| FAULTLAB_DOLLAR_CAP | 100 |
| FAULTLAB_INPUT_DOLLARS_PER_MILLION | 0.03 |
| FAULTLAB_OUTPUT_DOLLARS_PER_MILLION | 0.17 |
| FAULTLAB_PRICING_MODEL | openai/gpt-oss-120b |
| FAULTLAB_PRICING_VERIFIED | true |

Rates were checked against [W&B token pricing](https://wandb.ai/site/pricing/tokens/). The scheduler reserves the maximum 8,000 input and 2,000 output tokens at these rates before each model dispatch. Actual returned usage is recorded separately; missing billing metadata stays unknown. The dollar cap is per campaign, not a shared account or session limit.
...
The smoke check defaults to 32 output tokens. This explicit override allows GPT-OSS reasoning within the existing 2,000-token campaign ceiling. There is one generation, a 20-second timeout and no automatic generation retry. It verifies the completed call by reading it back from Weave; flush alone is insufficient. The setting overrides apply only to this command and do not edit credentials.
