...
import { RunSummary, campaignProposals } from '../src/components/RunSummary'
import { matrix as matrixFixture } from './fixtures'
import type { PolicyVersion } from '../src/generated/contracts'
const candidate: PolicyVersion = { ...baseline, version: 'policy-candidate', author: 'mechanic', decision: 'REJECTED', development_episode_ids: [episode.episode_id] }
it('summarises only stages that actually ran and never claims an unaccepted policy', () => {
  const records = validateExperiments({ ...experiments, counterexamples: [testedExperiment] })
  const view = render(h(RunSummary, { campaign, matrix: matrixFixture, experiments: records, policies: [baseline, candidate] }))
  expect(screen.getByText(/No recovery policy has been accepted in this campaign/)).toBeTruthy()
  expect(view.container.textContent).toContain('1 candidate policy generated by the Mechanic')
  expect(view.container.textContent).toContain('Candidate violated C1')
  expect(screen.getByText('No promotion batch qualified.')).toBeTruthy()
  expect(writes).toEqual([])
})
it('reports unreached stages as not run rather than leaving them blank', () => {
  const view = render(h(RunSummary, { campaign, matrix: matrixFixture, experiments: validateExperiments(experiments), policies: [baseline] }))
  expect(view.container.textContent).toContain('No qualifying repeated violation was recorded.')
  expect(view.container.textContent).toContain('No candidate policy was generated in this campaign.')
  expect(view.container.textContent).toContain('A candidate must repair every source-validation pair first.')
  expect(campaignProposals([baseline], matrixFixture)).toEqual([])
  expect(campaignProposals([candidate], matrixFixture)).toEqual([candidate])
  expect(writes).toEqual([])
})
